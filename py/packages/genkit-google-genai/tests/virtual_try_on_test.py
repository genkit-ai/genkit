...
def _media_part(url: str, part_type: str | None) -> Part:
    ...
    return Part.from_media(url, content_type='image/png', metadata=metadata)
...
class TestRequestShape:
    ...
    async def test_untagged_and_other_tags_are_ignored(self) -> None:
        ...
        await _generate(
            ...
            _request(
                parts=[
                    Part.from_text('dress her in this'),
...
class TestResponseShape:
    ...
    async def test_two_predictions_become_two_media_parts(self) -> None:
        ...
        assert [part.media.url for part in response.message.content] == [
    ...
    async def test_missing_mime_type_defaults_to_png(self) -> None:
        ...
        assert response.message.content[0].media.content_type == 'image/png'
