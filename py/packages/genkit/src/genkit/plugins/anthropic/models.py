        ...
        async with self.client.messages.stream(**params) as stream:
            async for chunk in stream:
                if chunk.type == 'content_block_start' and hasattr(chunk, 'content_block'):
                    ...
                    # anthropic's delta is a union of *Delta variants discriminated
                    # by `.type`; typed Any here so the runtime guards below aren't
                    # second-guessed by static narrowing.
