...
class OllamaModel:
    ...
    async def _chat_with_ollama(
        ...
        # ollama's `format` arg accepts either the '' / 'json' literal or a JSON
        # schema dict; request.output_format is loosely typed at the spec level.
        fmt: dict[str, Any] | Literal['', 'json']
        if request.output_schema:
            fmt = request.output_schema
        elif request.output_format:
            fmt = cast(Literal['', 'json'], request.output_format)
        else:
            fmt = ''
