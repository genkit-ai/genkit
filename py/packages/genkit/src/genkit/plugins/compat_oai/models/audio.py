...
class OpenAISTTModel:
    ...
    async def generate(self, request: ModelRequest, ctx: ActionRunContext) -> ModelResponse:
        ...
        # transcriptions.create(stream=False) returns Transcription | TranscriptionVerbose
        # | TranscriptionDiarized | str. _to_stt_response handles all of these via
        # isinstance/hasattr checks; the AsyncStream variant from the static return
        # type is unreachable when stream=False.
        return _to_stt_response(cast(Transcription | str, result))
