...
class BaseMiddleware(BaseModel):
    ...
    **Where to put things**
    ...
    def tools(self, ctx: MiddlewareContext) -> list[Action]:
        ...
        Called once per ``generate()`` with ``ctx.registry`` scoped to the call.
        Tools registered through that registry are torn down when the call ends.
...
def _derive_config_schema(cls: type[BaseMiddleware]) -> dict[str, Any]:
    ...
    schema. Framework services live on ``MiddlewareContext``, not the model.
    """
