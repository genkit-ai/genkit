...
def _session_id_of(snapshot: SessionSnapshot) -> str | None:
...
def _require_one_selector(snapshot_id: str | None, session_id: str | None) -> None:
...
def _stamp_store_fields(snapshot: SessionSnapshot, snapshot_id: str | None) -> None:
...
def _apply_save(existing: SessionSnapshot | None, snapshot_id: str | None, fn: SaveFn) -> SessionSnapshot | None:
    ...
    if next_snapshot is None:
        return None
    _stamp_store_fields(next_snapshot, snapshot_id)
    return next_snapshot


def _notify(subs: Subs, snapshot_id: str, status: SnapshotStatus | None) -> None:
...
class InMemorySessionStore(SessionStore[StateT], SnapshotSubscriber, Generic[StateT]):
    ...
    async def get_snapshot(
        ...
        *,
        ...
        async with self._lock:
            ...
            owned = [snap for snap in self._snapshots.values() if _session_id_of(snap) == session_id]
    ...
    async def save_snapshot(self, snapshot_id: str | None, fn: SaveFn) -> SessionSnapshot | None:
        async with self._lock:
            existing = self._snapshots.get(snapshot_id) if snapshot_id is not None else None
            next_snapshot = _apply_save(existing, snapshot_id, fn)
            if next_snapshot is None:
                return None
            self._snapshots[next_snapshot.snapshot_id] = next_snapshot.model_copy(deep=True)
            _notify(self._subs, next_snapshot.snapshot_id, next_snapshot.status)
            return next_snapshot

    async def on_snapshot_status_change(self, snapshot_id: str) -> asyncio.Queue[SnapshotStatus | None]:
        async with self._lock:
            return await _subscribe(self._subs, snapshot_id, self._snapshots.get(snapshot_id))
...
class FileSessionStore(SessionStore[StateT], SnapshotSubscriber, Generic[StateT]):
    ...
    def _read_sync(self, snapshot_id: str) -> SessionSnapshot | None:
    ...
    def _write_sync(self, snapshot: SessionSnapshot) -> None:
    ...
    def _read_session_sync(self, session_id: str) -> list[SessionSnapshot]:
    ...
    async def get_snapshot(
        ...
        *,
        ...
        async with self._lock:
            ...
            assert session_id is not None
            owned = await asyncio.to_thread(self._read_session_sync, session_id)
            return _select_leaf(owned, session_id, reject_ambiguous=self._reject_ambiguous)

    async def save_snapshot(self, snapshot_id: str | None, fn: SaveFn) -> SessionSnapshot | None:
        async with self._lock:
            existing = await asyncio.to_thread(self._read_sync, snapshot_id) if snapshot_id is not None else None
            next_snapshot = _apply_save(existing, snapshot_id, fn)
            if next_snapshot is None:
                return None
            await asyncio.to_thread(self._write_sync, next_snapshot)
            _notify(self._subs, next_snapshot.snapshot_id, next_snapshot.status)
            return next_snapshot

    async def on_snapshot_status_change(self, snapshot_id: str) -> asyncio.Queue[SnapshotStatus | None]:
        async with self._lock:
            current = await asyncio.to_thread(self._read_sync, snapshot_id)
            return await _subscribe(self._subs, snapshot_id, current)
