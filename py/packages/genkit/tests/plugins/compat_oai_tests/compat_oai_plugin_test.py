...
async def test_openai_runtime_clients_are_loop_local() -> None:
    ...
    plugin = OpenAI(api_key='test-key')
    # exercising cache semantics — the produced value is unused, so any factory
    # signature works at runtime.
    plugin._runtime_client = loop_local_client(lambda: object())  # type: ignore[arg-type, assignment]  # pyrefly: ignore[bad-argument-type]
