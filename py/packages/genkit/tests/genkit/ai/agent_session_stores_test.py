...
def make_snapshot(session_id: str, text: str, status: SnapshotStatus = SnapshotStatus.DONE, parent_id: str | None = None) -> SessionSnapshot:
...
@pytest.mark.asyncio
async def test_latest_state_store_in_memory():
    store = InMemoryLatestStateStore()
    await run_latest_state_store_test(store)


@pytest.mark.asyncio
async def test_latest_state_store_file(tmp_path):
    store = FileLatestStateStore(str(tmp_path))
    await run_latest_state_store_test(store)


async def run_latest_state_store_test(store):
    session_id = "sess-123"
