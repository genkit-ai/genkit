...
from genkit._core._typing import FinishReason, Part, Role


def test_tag_history_for_render_copies_messages() -> None:
    original = Message(role=Role.USER, content=[Part(text='hi')], metadata={'keep': True})
...
def test_apply_preamble_tags_tags_template_messages_and_strips_history_marker() -> None:
    history = Message(role=Role.USER, content=[Part(text='turn 1')], metadata={_HISTORY_TAG: True})
    system = Message(role=Role.SYSTEM, content=[Part(text='be helpful')])
...
def test_apply_preamble_tags_does_not_mutate_shared_prompt_messages() -> None:
    shared = Message(role=Role.SYSTEM, content=[Part(text='static system')])
...
async def test_prompt_agent_does_not_persist_system_preamble() -> None:
    ...
    pm.responses.append(
        ModelResponse(
            ...
            message=Message(role=Role.MODEL, content=[Part(text='ok')]),
        )
    )
...
async def test_prompt_agent_multi_turn_session_has_no_accumulated_preamble() -> None:
    ...
    pm.responses.extend([
        ModelResponse(
            finish_reason=FinishReason.STOP,
            message=Message(role=Role.MODEL, content=[Part(text='first')]),
        ),
        ModelResponse(
            finish_reason=FinishReason.STOP,
            message=Message(role=Role.MODEL, content=[Part(text='second')]),
        ),
    ])
