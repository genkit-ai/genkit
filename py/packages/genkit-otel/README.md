...
It provides `GenAiInstrumentation`: OpenTelemetry GenAI semantic-conventions instrumentation emitting `gen_ai.*` spans and metrics.

Pass `tracer` to mint on that tracer instead of the process-global one.
