...
async def test_googleai_runtime_clients_are_loop_local(mock_client_ctor: MagicMock) -> None:
    ...
    def _other_thread() -> None:
        async def _get_client() -> MagicMock:
            # _runtime_client is patched above to return MagicMock instances.
            return cast(MagicMock, plugin._runtime_client())
...
def test_gemini_config_schema() -> None:
    ...
    config = GeminiConfigSchema.model_validate({'temperature': 0.7, 'max_output_tokens': 1000})
