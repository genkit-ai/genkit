...
async def test_streaming_tool_request() -> None:
    ...
    tool_block = ToolUseBlock(type='tool_use', id='tool_abc', name='get_weather', input={})
    chunks = [
        RawContentBlockDeltaEvent(
            type='content_block_delta', index=0, delta=TextDelta(type='text_delta', text='Let me check.')
        ),
        RawContentBlockStartEvent(type='content_block_start', index=1, content_block=tool_block),
        RawContentBlockDeltaEvent(
