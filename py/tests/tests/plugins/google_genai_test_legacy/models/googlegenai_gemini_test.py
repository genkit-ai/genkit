...
async def test_generate_media_response(mocker: MockerFixture, version: str) -> None:
    ...
    request = ModelRequest(
        ...
        config={'response_modalities': modalities},  # pyright: ignore[reportArgumentType]
    )
...
def test_gemini_model__normalize_config_canonicalizes_aliases(
    ...
    dumped = gemini_model_instance._normalize_config_to_dict(config)  # type: ignore[arg-type]  # pyrefly: ignore[bad-argument-type]
...
async def test_gemini_model__camelcase_code_execution_translates_to_tool(
    ...
    request = ModelRequest(
        ...
        config=GeminiConfigSchema.model_validate({'code_execution': True}).model_dump(),  # pyright: ignore[reportArgumentType]
    )
