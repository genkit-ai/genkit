...
Ollama requires object-typed tool input schemas. Wrap primitive inputs in a
Pydantic model — the plugin raises `ValueError` if a tool advertises a
non-object schema.
...
See [`py/samples/ollama`](../../samples/ollama) for a runnable sample covering
