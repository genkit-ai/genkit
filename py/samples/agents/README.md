...
| `02_define_agent_no_store.py` | Client-managed state: capture `chat.messages`/`chat.state`/`chat.artifacts` and resume via `chat(messages=…, state=…, artifacts=…)`. |
...
| `08_graceful_failure.py` | A failed turn (`finish_reason=failed`) stays contained; the next send resumes from the last good snapshot. |
...
| `12_abort_client_and_server.py` | Client-side `turn.abort()` vs server-side `chat.abort()`. |
| `13_deadline_timeout.py` | Put a deadline on a turn with `asyncio.timeout(...)`; the turn detaches and surfaces `TimeoutError`. |
| `14_abort_failure_store_drift.py` | Three ways a turn ends early — client `turn.abort()`, server `task.abort()`, FAILED — plus reloading with `load_chat` to resync a detached turn's reply. |
