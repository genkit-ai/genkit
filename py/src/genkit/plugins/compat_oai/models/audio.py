...
def _to_stt_response(result: Transcription | TranscriptionVerbose | TranscriptionDiarized | str) -> ModelResponse:
    ...
    All non-str result types share a `.text` attribute.
...
class OpenAISTTModel:
    ...
    async def generate(self, request: ModelRequest, ctx: ActionRunContext) -> ModelResponse:
        ...
        # transcriptions.create(stream=False) statically returns a wider union
        # that includes AsyncStream; assert it away so static and runtime agree.
        assert isinstance(result, (Transcription, TranscriptionVerbose, TranscriptionDiarized, str)), (
            f'unexpected stt response type {type(result).__name__}'
        )
