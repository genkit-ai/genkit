...
class MessageConverter:
    ...
    def _get_openai_role(cls, role: Role | str) -> str:
        ...
        if isinstance(role, Role):
            return cls._openai_role_map.get(role, str(role.value))
    ...
    def tool_call_to_genkit(
        ...
        """
        # tool_call comes through as either a parsed dict (e.g. streaming
        # accumulator state) or an OpenAI SDK ChatCompletionMessageToolCall;
        # branch on dict so each arm has a concrete shape to read from.
        if isinstance(tool_call, dict):
            # narrow isinstance's bare dict[Unknown, Unknown] to the actual key/value shape
