...
class OllamaModel:
    ...
    async def _chat_with_ollama(
        ...
        # ollama's `format` accepts only '' / 'json' as a string or a JSON
        # schema dict; map the only meaningful string value through explicitly
        # so unexpected outputs (e.g. 'text') become an unconstrained format.
        fmt: dict[str, Any] | Literal['', 'json']
...
def _convert_parameters(input_schema: dict[str, object]) -> ollama_api.Tool.Function.Parameters | None:
    ...
    # `$defs` is a valid pydantic alias but not a Python identifier, so the
    # synthesized __init__ surface trips type checkers; pass it through kwargs.
    schema = ollama_api.Tool.Function.Parameters(**{'$defs': None})
