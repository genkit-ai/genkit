                    ...
                    if isinstance(delta, TextDelta):
                        ctx.send_chunk(
                            ModelResponseChunk(
                                role=Role.MODEL,
                                index=0,
                                content=[Part(root=TextPart(text=delta.text))],
                            )
                        )
                    elif isinstance(delta, InputJSONDelta):
                        if chunk.index in pending_tools:
                            pending_tools[chunk.index]['input_json'] += delta.partial_json

                elif isinstance(chunk, RawContentBlockStopEvent):
                    if chunk.index in pending_tools:
                        tool_info = pending_tools.pop(chunk.index)
                        tool_input: object = {}
