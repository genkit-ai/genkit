...
class VertexAI(Plugin):
    ...
    def _resolve_evaluator(self, name: str) -> Action | None:
        ...
        # cast keeps the element typed as the metric union rather than the
        # StrEnum base class some checkers widen the literal to.
        actions = create_vertex_evaluators(
            registry,
            [cast(VertexAIEvaluationMetric, metric_type)],
            project_id=self._project,
            location=self._location,
        )
