...
class VeoModel:
    ...
    async def start(self, request: ModelRequest, ctx: ActionRunContext) -> Operation:
        ...
        # request.config arrives as a loose dict when callers pass raw kwargs;
        # narrow it to GenerateVideosConfigDict explicitly rather than relying
        # on the SDK's untyped dict acceptance.
        config: genai_types.GenerateVideosConfigDict | None = (
            cast(genai_types.GenerateVideosConfigDict, request.config) if isinstance(request.config, dict) else None
        )
