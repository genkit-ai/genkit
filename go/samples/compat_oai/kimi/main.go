...
// This sample demonstrates the Kimi plugin for Moonshot AI's models: a
// streaming flow that generates a joke with a model pinned through
// kimi.ModelRef and its typed config.
//
// Run it:
//
//	export KIMI_API_KEY=...
...
// one-line change. Thinking is on by default for the K2 generation, so the
// config turns it off for a quick conversational answer.
var model = kimi.ModelRef("kimi-k2.6", &kimi.ChatConfig{
