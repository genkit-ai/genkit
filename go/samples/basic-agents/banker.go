...
var transferMoney *ai.ResumableToolAction[TransferInput, *TransferOutput, Confirmation]
...
func defineBankerAgent(g *genkit.Genkit) *aix.Agent[any] {
	...
	// transferMoney is a resumable tool: rather than always returning a
	...
	return genkitx.DefinePromptAgent(g, name,
		...
		aix.WithDescription[any]("Money transfer assistant (resumable tool + human approval)"),
	)
}
