...
// ToolErrors is a middleware that returns tool errors to the model instead of
// failing the generation. The model receives the error as the tool's
// response, {"error": "<message>"}, and can correct its input and try again.
//
...
// before it becomes a response, wherever ToolErrors sits in the chain.
//
...
//	    ai.WithUse(&middleware.ToolErrors{}),
//	)
...
func (t ToolErrors) Name() string { return provider + "/toolErrors" }

// New implements [ai.Middleware], hooking each tool-loop iteration.
func (t ToolErrors) New(ctx context.Context) (*ai.Hooks, error) {
	return &ai.Hooks{WrapGenerate: returnToolErrors(t.covers)}, nil
}

func (t ToolErrors) covers(name string) bool {
