...
//
// Only server-managed sub-agents (those with a session store) are resumable.
...
// handle, and resumes the snapshot behind it. Only server-managed sub-agents
// are resumable: a client-managed delegation settles inline and leaves
// nothing durable a handle could name.
//
...
func (a *Agents) runResume(ctx context.Context, st *agentsState, in resumeInput, background bool) (delegationResult, error) {
	...
	if isClientManaged(agent) {
		...
		return delegationResult{Response: fmt.Sprintf(
			"Error: agent %q manages its state on the client and its delegations are not resumable; delegate the task again. Only sub-agents with a session store leave resumable task handles.", ref.Name)}, nil
	}
	return a.resumeFromStore(ctx, ref, st, agent, invocationNum, in, snapshotID, background)
}
