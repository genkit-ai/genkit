...
func (f Filesystem) New(ctx context.Context) (*ai.Hooks, error) {
	...
	if err != nil {
		return nil, status.Errorf(status.ErrInternal, "filesystem middleware: resolve %q: %w", f.RootDir, err)
	}
	root, err := os.OpenRoot(abs)
	if err != nil {
		return nil, status.Errorf(status.ErrFailedPrecondition, "filesystem middleware: open root %q: %w", abs, err)
	}
