...
to use another compatible endpoint, such as DeepSeek's beta endpoint.
...
nests inside it. `deepseek.ModelRef` carries the config with the model ID.
Thinking is on by default, so turning it off is the common case:
