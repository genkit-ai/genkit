...
func TestModelRefAndConfigSchema(t *testing.T) {
	...
	for _, key := range []string{"temperature", "maxOutputTokens", "stopSequences", "reasoningEffort", "searchParameters", "version"} {
		if props[key] == nil {
...
// max_completion_tokens xAI wants instead of the deprecated max_tokens), and
// the xAI extras ride along, all in one Generate call.
func TestModelRefConfigReachesTheWire(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := body.SearchParameters["mode"]; got != "on" {
			t.Errorf("search_parameters.mode = %v, want %q", got, "on")
		}
		if got := body.SearchParameters["return_citations"]; got != true {
			t.Errorf("search_parameters.return_citations = %v, want true", got)
		}
		if got := body.SearchParameters["from_date"]; got != "2026-01-01" {
			t.Errorf("search_parameters.from_date = %v, want %q", got, "2026-01-01")
		}
		if got := body.SearchParameters["max_search_results"]; got != float64(5) {
			t.Errorf("search_parameters.max_search_results = %v, want 5", got)
		}
		sources, ok := body.SearchParameters["sources"].([]any)
		if !ok || len(sources) != 1 {
			t.Fatalf("search_parameters.sources = %#v, want one source", body.SearchParameters["sources"])
		}
		if got := sources[0].(map[string]any)["type"]; got != "x" {
			t.Errorf("search_parameters.sources[0].type = %v, want %q", got, "x")
		}
...
// TestJSONConfigCarriesSearchParameters pins the JSON contract of the search
// parameters: a map config (what the Dev UI and cross-runtime callers send)
// passes schema validation, its camelCase keys reach the wire as snake_case,
// and the source objects pass through with the API's own keys.
func TestJSONConfigCarriesSearchParameters(t *testing.T) {
	...
	if _, err := genkit.Generate(ctx, g,
		...
		ai.WithConfig(map[string]any{
			"searchParameters": map[string]any{
				"mode":    "auto",
				"toDate":  "2026-08-01",
				"sources": []any{map[string]any{"type": "web", "excluded_websites": []any{"example.com"}}},
			},
		}),
	); err != nil {
		t.Fatalf("Generate() error = %v", err)
	...
	if got["mode"] != "auto" {
		t.Errorf("search_parameters.mode = %v, want %q", got["mode"], "auto")
	}
	if got["to_date"] != "2026-08-01" {
		t.Errorf("search_parameters.to_date = %v, want %q", got["to_date"], "2026-08-01")
	}
	sources, ok := got["sources"].([]any)
	if !ok || len(sources) != 1 {
		t.Fatalf("search_parameters.sources = %#v, want one source", got["sources"])
	}
	source := sources[0].(map[string]any)
	if source["type"] != "web" {
		t.Errorf("search_parameters.sources[0].type = %v, want %q", source["type"], "web")
	}
	if _, ok := source["excluded_websites"].([]any); !ok {
		t.Errorf("search_parameters.sources[0] = %#v, want excluded_websites passed through", source)
	}
}
