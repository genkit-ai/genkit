...
// DefineXTool defines a tool with a simplified function signature, registers it
// as a [core.Action] of type Tool, and returns an [aix.Tool].
//
// Experimental: This API is under active development and may change in any
// minor version release.
//
// Unlike [DefineTool], the function receives a plain [context.Context] instead
// of [ai.ToolContext]. Use [tool.AttachParts] inside the function to return
// additional content parts alongside the output.
//
// For tools that don't need to be registered (e.g., dynamically created tools),
...
func DefineXTool[In, Out any](g *Genkit, name, description string, fn aix.ToolFunc[In, Out], opts ...ai.ToolOption) *aix.Tool[In, Out] {
	return aix.DefineTool(g.reg, name, description, fn, opts...)
}
