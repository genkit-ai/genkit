...
Delegations that settle leave a resumable handle behind. Every delegation result and background-task report carries a `taskId`, and the shared `resume_subagent` tool spends it: a failed or aborted task continues from its last saved progress (with no `instructions` the committed turn is re-attempted as it stood; with `instructions` the retry is steered), and a completed task accepts follow-up instructions inside the sub-agent's own session, so pressing on never repeats finished work. With `Async` set, `resume_subagent` also takes `background: true` and returns a fresh task ID in the same session. A server-managed sub-agent's handles survive across turns and process restarts; a client-managed sub-agent's state is held in memory for the current turn only, under a `<agent>:mem-<n>` handle. An expired task (its worker died) is fenced with an abort and resumed from the session's last committed snapshot; a background launch that died before committing anything has nothing saved, and the tool says to delegate again.
