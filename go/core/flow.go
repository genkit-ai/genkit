...
func NewFlow[In, Out any](name string, fn Func[In, Out]) *Flow[In, Out, struct{}] {
	return &Flow[In, Out, struct{}]{NewAction(name, api.ActionTypeFlow, nil, func(ctx context.Context, input In) (Out, error) {
		fc := &flowContext{
			flowName: name,
		}
...
func NewStreamingFlow[In, Out, Stream any](name string, fn StreamingFunc[In, Out, Stream]) *Flow[In, Out, Stream] {
	return &Flow[In, Out, Stream]{NewStreamingAction(name, api.ActionTypeFlow, nil, func(ctx context.Context, input In, cb func(context.Context, Stream) error) (Out, error) {
		fc := &flowContext{
			flowName: name,
		}
		ctx = flowContextKey.NewContext(ctx, fc)
