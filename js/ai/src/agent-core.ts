...
/**
 * What a turn's stream said about how the turn ended. `buildResponse` reads
 * it when the output alone cannot say whether the turn committed; see
 * `sendStream`. `settled` resolves once the stream has been drained.
 */
...
export class AgentChatImpl<State = unknown> implements AgentChat<State> {
  ...
  private buildResponse(
    ...
    return (async (): Promise<AgentResponse<State>> => {
      ...
      if (terminal && committed && snapshotIdBeforeTurn === undefined) {
        ...
        if (evidence.turnEnd) {
          committed = evidence.turnEnd.snapshotId !== undefined;
        }
      }
      if (
        terminal &&
  ...
  sendStream(
    ...
    // committed failed turn from a rolled-back one.
    ...
    (async () => {
      try {
        for await (const raw of rawStream) {
          if (raw.turnEnd) evidence.turnEnd = raw.turnEnd;
          queue.push(raw);
        }
        queue.close();
      } catch (e) {
