 ...
 * what its run committed; a `pending` or `aborting` one is rejected while
 * its detached invocation is still writing to it. For client-managed agents (no store) provide the
 * full `state`.
 *
