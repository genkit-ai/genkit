...
describe('generate failures', () => {
  ...
  describe('thrown (the default)', () => {
    ...
    it("hands a generate hook the tool's own error, as a caller gets it", async () => {
      const boom = new Error('db password rejected');
      defineTool(
        registry,
        { name: 'badTool', description: 'bad' },
      ...
      const seen: unknown[] = [];
      const watching = generateMiddleware({ name: 'watching' }, () => ({
        generate: async (envelope, ctx, next) => {
          try {
            return await next(envelope, ctx);
          } catch (e) {
      ...
      await assert.rejects(generate(registry, options), (e) => e === boom);
      assert.ok(
        seen.length > 0 && seen.every((e) => e === boom),
        'every hook frame catches the tool error itself'
      );

      // A hook that lets the error through keeps the loop's partial and its
      // classification of the failure.
      modelCalls = 0;
      const res = await generateReturning(options);
      assert.strictEqual(res.finishReason, 'failed');
      assert.deepStrictEqual(res.error, {
        status: 'INTERNAL',
        message: 'tool "badTool" failed: db password rejected',
