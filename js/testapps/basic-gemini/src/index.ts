...
ai.defineFlow('deep-research-code-execution', async (_, { sendChunk }) => {
  let { operation } = await ai.generate({
    model: googleAI.model('deep-research-preview-04-2026'),
    prompt:
      'Start with 1. Add 3, then divide by 2. Take that answer, and continue ' +
      'adding 3 and dividing by 2 to each successive answer and tell me the ' +
      'first 20 terms in the sequence. Be succinct. Not 20 pages of pure math ' +
      'theory and philosophy. Just the first 20 terms of this sequence as ' +
      'generated by using code execution.',
    config: {
      codeExecution: true,
    },
  });
...
const getCharacterName = ai.defineTool(
  {
    name: 'getCharacterName',
    description: 'Gives the name of a main character',
    inputSchema: z.object({
      longText: z.string().describe('A very long paragraph of text.'),
      nestedData: z.object({
        arrayItems: z.array(z.string()),
        id: z.number(),
      }),
    }),
  },
  async (input) => {
    return 'Buttons the dog';
  }
);
...
ai.defineFlow(
  {
    name: 'streaming-tool-calling',
    inputSchema: z.string().default('Tell me a story.'),
    outputSchema: z.string(),
    streamSchema: z.any(),
  },
  async (promptText, { sendChunk }) => {
    const { response, stream } = ai.generateStream({
      model: googleAI.model('gemini-flash-latest'),
      config: {
        temperature: 1,
      },
      tools: [getCharacterName],
      prompt: `${promptText} Please call the getCharacterName tool with a very long story of at least 300 words in the longText field, and some nested array items. Then modify the original story to suit the new main character name, and give me the final story.`,
    });
...
ai.defineFlow('nano-banana-2-lite', async (_) => {
  const { media } = await ai.generate({
    model: googleAI.model('gemini-3.1-flash-lite-image'),
    prompt: 'Generate an image of the CN Tower. ',
    config: {
      imageConfig: {
        aspectRatio: '1:4',
        imageSize: '1K',
