...
ai.defineFlow('get-image', async (q) => {
  const { text } = await ai.generate({
    prompt: `get the test image and describe it`,
    tools: ['test-mcp-manager:tool/inmemory/get_test_image'],
  });

  return text;
});

ai.defineFlow('test-error-tool', async (q) => {
  const { text } = await ai.generate({
    prompt: `call the error_tool. When it fails, explain what happened.`,
    tools: ['test-mcp-manager:tool/inmemory/error_tool'],
  });
