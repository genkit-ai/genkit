...
- Streaming works only in the non-obvious form, with no type error to warn you. The dispatcher selects middleware arity by `Function.length` (`js/ai/src/generate.ts:414-433`): the streaming callback (`ctx`, carrying `onChunk`) reaches the middleware only in the 3-arg `(req, ctx, next)` form, not the 2-arg `(req, next)` one. Both arities satisfy the middleware type, so a `(req, next) => …` mock compiles clean, passes a non-streaming test, then silently drops every chunk under `generateStream`. The 3-arg form is correct — but it's an implicit, untyped convention the app author has to know. A helper *we* ship writes it once and hides the footgun; with middleware the cost lands on every hand-rolled mock.
